-- Fix seed function with correct enum values
...
BEGIN
  ...
  -- Always ensure user has a clinic and role
  ...
  ELSE
    -- Ensure user has role in clinic
    IF NOT EXISTS (SELECT 1 FROM user_roles WHERE user_id = uid AND clinic_id = clinic) THEN
      INSERT INTO user_roles (user_id, clinic_id, role) VALUES (uid, clinic, 'admin');
    END IF;
  END IF;
  ...
  IF (SELECT count(*) FROM clients WHERE clinic_id = clinic) = 0 THEN
    ...
    -- 15 service types (using correct enum values: consulta, cirurgia, exame, banho, tosa, hospedagem)
    IF (SELECT count(*) FROM service_types WHERE clinic_id=clinic) = 0 THEN
      ...
      FROM (VALUES
        ...
        ('Banho Simples','banho',45.00,60),
        ('Banho e Tosa','tosa',85.00,60),
        ('Tosa Higiênica','tosa',35.00,60),
        ('Cirurgia Castração','cirurgia',350.00,120),
        ('Cirurgia Geral','cirurgia',500.00,120),
        ('Hospedagem Diária','hospedagem',120.00,30),
